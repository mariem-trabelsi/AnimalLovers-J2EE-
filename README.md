...
Le projet contient trois catégories principales de composants : **les données** (dossiers `data`, `people`, `videos` pour les datasets et médias de test), **les modèles** (dossier `models` stockant les poids pré-entraînés et fine-tunés), et **les résultats** (dossier `results` pour les sorties d'inférence).
